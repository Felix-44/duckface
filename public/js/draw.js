...
function drawScrollingImage(context, image, x = 0, y = 0, z = 1) {
  const cameraX =
    x + camera.pixelX * z >= 0
      ? (x + camera.pixelX * z) % 800
      : 800 - (-(x + camera.pixelX * z) % 800);
  const cameraY =
    y + camera.pixelY * z >= 0
      ? (y + camera.pixelY * z) % 600
      : 600 - (-(y + camera.pixelY * z) % 600);
...
function drawGameButtons(context) {
  if (
    state.completed &&
    Date.now() - (sceneStartTime + sceneCompletionTime) > 2000
  ) {
    drawButton("complete-try-again", "Try again (R)", 0.5, 0.34, () => {
      resetScene();
    });

    drawButton("next", "Next (Enter)", 0.5, 0.5, () => {
      openNext();
    });
    ...
    state.failed === true &&
    Date.now() - (sceneStartTime + sceneFailedTime) > 2000
  ) {
...
function drawButton(id, text, x = 0.5, y = 0.5, callback) {
  ...
  context.moveTo(
    right - 4 + randoms.nextValue,
    bottom - 12 + randoms.nextValue
  );
  context.lineTo(
    right - 6 + randoms.nextValue,
    bottom - 10 + randoms.nextValue
  );
  context.stroke();

  // button click
  if (
    controls.mousePixelX >= left &&
    controls.mousePixelX <= right &&
    controls.mousePixelY >= top &&
    controls.mousePixelY <= bottom
  ) {
    ...
    controls.touchX >= left &&
    controls.touchX <= right &&
    controls.touchY >= top &&
    controls.touchY <= bottom
  ) {
    // touch
...
function drawTitleScreen() {
  ...
  drawTitle({
    text:
      "Level " +
      (album.scenes.findIndex((s) => s.id === scene.id) + 1) +
      " of " +
      album.scenes.length,
    fontSize: 30,
    fillStyle: "#ffffff",
  ...
  if (scene.themeSong) {
    drawTitle({
      text: "Music: " + themes[scene.themeSong].title,
      fontSize: 25,
      fillStyle: "#ffffff",
      y: 0.85,
      lineWidth: 1.5,
    ...
    drawTitle({
      text: "by: " + themes[scene.themeSong].authors,
      fontSize: 25,
      fillStyle: "#ffffff",
      y: 0.9,
      lineWidth: 1.5,
...
function drawBackground(context, canvas) {
  ...
  context.globalAlpha = Math.min(
    1,
    (Date.now() - (sceneStartTime + sceneCompletionTime)) / 1000
  );
  ...
  for (let r = canvas.width / 2 + 80; r > step; r -= step) {
    ...
    context.arc(
      canvas.width / 2 +
        Math.sin(r / step + (Date.now() - sceneLoadedTime) / 500) * 8,
      canvas.height / 2 +
        Math.cos(r / step + (Date.now() - sceneLoadedTime) / 500) * 8,
      r,
      0,
      Math.PI * 2
    );
...
function drawSun(
  id,
  context,
  { x = 0.5, y = 0.5, radius = 80, color = "#f8f890", stroke = "#f8f890" }
) {
  if (!effects.canvas[id]) {
    effects.canvas[id] = document.createElement("canvas");
    effects.canvas[id].width = context.canvas.width;
  ...
  if (effects.redraw) {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      bgEffectContext.setLineDash([
        4 + Math.random() * 4,
        2 + i / 2,
        2,
        2,
        2 + Math.random() * 16,
        2 + i / 2 + randoms.nextValue,
      ]);
      bgEffectContext.beginPath();
      bgEffectContext.arc(
...
function drawClouds(
  ...
  if (effects.redraw) {
    ...
    for (let i = 0; i < clouds; i++) {
      ...
      const y =
        Math.floor(Math.random() * effects.canvas[id].height * yLimit) - 50;

      bgEffectContext.beginPath();
      // bgEffectContext.rect(x, y, width, height);
      bgEffectContext.moveTo(x + randoms.nextValue, y + randoms.nextValue);
      bgEffectContext.lineTo(
        x + randoms.nextValue + width,
        y + randoms.nextValue
      );
      bgEffectContext.lineTo(
        x + randoms.nextValue + width,
        y + height + randoms.nextValue
      );
      bgEffectContext.lineTo(
        x + randoms.nextValue,
        y + height + randoms.nextValue
      ...
      for (let j = 0; j < bumbs; j++) {
        ...
        const by =
          Math.ceil(y - bheight + (Math.random() * bheight) / 3) +
          (j * bheight) / 5;

        bgEffectContext.beginPath();
        bgEffectContext.moveTo(
          bx + randoms.nextValue,
          by + randoms.nextValue + bheight
        );
        bgEffectContext.lineTo(bx + randoms.nextValue, by + randoms.nextValue);
        bgEffectContext.lineTo(
          bx + randoms.nextValue + bwidth,
          by + randoms.nextValue
        );
        bgEffectContext.lineTo(
          bx + randoms.nextValue + bwidth,
          by + randoms.nextValue + bheight
...
function drawRain(
  id,
  context,
  { drops = 200, color = "rgba(255,255,255,0.5)", vector = { x: 0, y: 8 } }
) {
  if (!effects.canvas[id]) {
    effects.canvas[id] = document.createElement("canvas");
    effects.canvas[id].width = context.canvas.width;
  ...
  if (effects.redraw) {
    ...
    for (let i = 0; i < drops; i++) {
      ...
      bgEffectContext.moveTo(x + randoms.nextValue, y + randoms.nextValue);
      bgEffectContext.lineTo(
        x + vector.x + randoms.nextValue,
        y + vector.y + randoms.nextValue
      );
      bgEffectContext.moveTo(x + randoms.nextValue, y + randoms.nextValue);
      bgEffectContext.lineTo(
        x + vector.x + randoms.nextValue,
...
function drawTileLayer(layer, context, canvas) {
  ...
  if (tileLayers.update) {
    if (
      ...
      tileLayers[layer].context.clearRect(
        0,
        0,
        tileLayers[layer].canvas.width,
        tileLayers[layer].canvas.height
      );
    }
  ...
  sprites
    .filter(
      (sprite) =>
        ...
        sprite.x <= camera.x + canvas.width &&
        sprite.y <= camera.y + canvas.height
    )
    .forEach((sprite) => {
      randoms.save();
...
function drawSprite(sprite, context) {
  ...
  if (!animationFrame) {
    throw new Error(
      `Frame ${sprite.frame} not found in animation ${sprite.animation}`
    );
  }
  ...
  if (!frame) {
    throw new Error(
      `Frame ${animationFrame.frame} not found in sprite type ${sprite.typeId}`
    );
  }
  ...
  frame.forEach((pathData) => {
    const path = type.paths[pathData.path];

    if (path) {
      if (pathData.x || pathData.y) {
        context.translate(
          (pathData.x || 0) * sprite.width,
          (pathData.y || 0) * sprite.width
        );
      }
      ...
      if (path.fill) {
        if (
          ...
          Math.floor(sprite.damageCounter / 4) % 2 === 0
        ) {
          context.fillStyle = "#ffffff";
      ...
      throw new Error(
        `Missing path ${pathData.path} in frame ${animationFrame.frame}`
      );
    }
  });
  ...
  if (tile.shape === SHAPES.BOX) {
    ...
    if (tile.layer !== LAYERS.BACKGROUND && tile.borders.t && tile.borders.l) {
      if (tile.height > 8 && tile.width > 9) {
        context.strokeStyle = "#ffffff";
        context.beginPath();
        context.moveTo(6 + randoms.nextValue, 4 + randoms.nextValue);
        context.lineTo(
    ...
    if (tile.materialId === tileMaterials.ice.id && tile.height > 24) {
      ...
      context.beginPath();
      context.moveTo(4 + randoms.nextValue, 4 + randoms.nextValue);
      context.lineTo(tile.width - 4 + randoms.nextValue, 4 + randoms.nextValue);
      context.lineTo(
        tile.width - 4 + randoms.nextValue,
    ...
    context.beginPath();
    context.moveTo(randoms.x(tile.x, tile.y), randoms.y(tile.x, tile.y));
    context.lineTo(
...
function drawText(text, y, fillStyle, strokeStyle) {
  ...
  context.strokeText(
    text,
    canvas.clientWidth / 2 - textWidth / 2,
...
function drawCaptions() {
  if (scene.captions.length > 0 && showCaptions) {
    ...
    scene.captions
      .filter(
        (caption) =>
          !caption.delay || caption.delay < now - gameState.stateStartTime
      )
      .forEach((caption) => {
        drawCaption(caption);
      });
  ...
  if (isPlayerReady) {
    if (mode.pause) {
      ...
      drawCaption({
        text: `Oh, ${
          Date.now() - (sceneStartTime + sceneFailedTime) > 1700 ? "F" : "D"
        }uck`,
        y: 0.5,
      });
    ...
    if (sceneCompletionTime !== -1) {
      drawCaption({
        text:
          "Time: " + Math.round(sceneCompletionTime / 10) / 100 + " seconds",
        y: 0.8,
      });
      ...
      if (sceneBestTime === undefined) {
        ...
        drawCaption({
          text: `New speed record! You were ${
            Math.round((sceneBestTime - sceneCompletionTime) / 10) / 100
          } seconds faster.`,
          y: 0.9,
        });
      } else {
        drawCaption({
          text:
            "Your best time: " +
            Math.round(sceneBestTime / 10) / 100 +
            " seconds",
          y: 0.9,
        });
      }
    ...
    if (sceneBestTime) {
      drawCaption({
        text: "Best time: " + Math.round(sceneBestTime / 10) / 100 + " seconds",
        y: 0.025,
        fillStyle: "#fff0d0",
      });
    }
...
function drawCaption({
  ...
  if (showBox) {
    ...
    context.lineTo(
      right + padding + randoms.nextValue,
      top + randoms.nextValue
    );
    context.lineTo(
      right + padding + randoms.nextValue,
      bottom + padding + randoms.nextValue
    );
    context.lineTo(
      left - padding + randoms.nextValue,
      bottom + padding + randoms.nextValue
    );
    context.closePath();
    context.stroke();
...
function drawEnd() {
  ...
  drawTitle({
    text: "You have reached the end of",
    fontSize: 40,
    fillStyle: "#ffffff",
    y: 0.2,
  });
  ...
  const bestTimes = JSON.parse(
    localStorage.getItem("duckface-best-times") || "{}"
  );
  let completedScenes = 0;
  let albumTime = 0;
  album.scenes.forEach((scene) => {
    scene.bestTime = -1;
    for (const time in bestTimes) {
  ...
  drawTitle({
    text: `Levels completed: ${completedScenes} / ${album.scenes.length}`,
    fontSize: 30,
    fillStyle: "#ffffff",
    y: 0.5,
  });
  drawTitle({
    text: `Best times total: ${Math.round(albumTime / 10) / 100} seconds`,
    fontSize: 30,
    fillStyle: "#ffffff",
    y: 0.6,
  });
  ...
  if (completedScenes < album.scenes.length) {
    drawTitle({
      text: "Good job. Now, go and finish the rest of the levels, you cheat!",
      fontSize: 20,
      fillStyle: "#ffffff",
      y: 0.8,
    });
    ...
    drawTitle({
      text: "Well done! You are the winner of everything!",
      fontSize: 20,
      fillStyle: "#ffffff",
      y: 0.8,
    });
...
function drawEditorStuff() {
  ...
  scene.tiles.forEach((tile) => {
    if (
      controls.mouseX >= tile.x &&
      controls.mouseX <= tile.x + tile.width &&
      controls.mouseY >= tile.y &&
      controls.mouseY <= tile.y + tile.height
    ) {
      canvas.style.cursor = "pointer";
    }
  });
  ...
  sprites.forEach((sprite) => {
    if (
      controls.mouseX >= sprite.x &&
      controls.mouseX <= sprite.x + sprite.width &&
      controls.mouseY >= sprite.y &&
      controls.mouseY <= sprite.y + sprite.height
    ) {
      canvas.style.cursor = "pointer";
    }
  });
  ...
  editor.idsOfSelectedSprites.forEach((id) => {
    ...
    if (sprite) {
      const spriteOrigin = scene.sprites.find((sprite) => sprite.id === id);
      const type = spriteTypes[sprite.typeId];
      ...
      if (type.shape === SHAPES.BOX) {
        ...
        context.beginPath();
        context.moveTo(
          spriteOrigin.x + spriteOrigin.width / 2,
          spriteOrigin.y + spriteOrigin.height / 2
        );
        context.lineTo(
...
function drawResizeHandles({
  ...
  if (
    movable &&
    controls.mousePixelX >= rect.x &&
    controls.mousePixelX <= rect.x + rect.width &&
    controls.mousePixelY >= rect.y &&
    controls.mousePixelY <= rect.y + rect.height
  ) {
  ...
  const radius = Math.max(
    4,
    Math.min(14, Math.min(rect.width, rect.height) / 3)
  );
  ...
  if (resizable) {
    drawResizeHandle({
      ...
      radius,
      cursor: "nwse-resize",
      resizeDirections: [DIRECTION.TOP, DIRECTION.LEFT],
    });
    drawResizeHandle({
      context,
      ...
      radius,
      cursor: "row-resize",
      resizeDirections: [DIRECTION.TOP],
    });
    drawResizeHandle({
      context,
      x: rect.x + rect.width,
      ...
      radius,
      cursor: "nesw-resize",
      resizeDirections: [DIRECTION.TOP, DIRECTION.RIGHT],
    });
    ...
    drawResizeHandle({
      ...
      radius,
      cursor: "col-resize",
      resizeDirections: [DIRECTION.LEFT],
    });
    drawResizeHandle({
      context,
      x: rect.x + rect.width,
      ...
      radius,
      cursor: "col-resize",
      resizeDirections: [DIRECTION.RIGHT],
    });
    ...
    drawResizeHandle({
      ...
      radius,
      cursor: "nesw-resize",
      resizeDirections: [DIRECTION.BOTTOM, DIRECTION.LEFT],
    });
    drawResizeHandle({
      context,
      x: rect.x + rect.width / 2,
      ...
      radius,
      cursor: "row-resize",
      resizeDirections: [DIRECTION.BOTTOM],
    });
    drawResizeHandle({
      context,
      x: rect.x + rect.width,
      ...
      radius,
      cursor: "nwse-resize",
      resizeDirections: [DIRECTION.BOTTOM, DIRECTION.RIGHT],
    });
  }
...
function drawSpriteInfo(context) {
  sprites.forEach((sprite) => {
    ...
    sprite.collisionData.boxes.forEach((box) => {
      context.strokeRect(box.l, box.t, box.r - box.l, box.b - box.t);
    });
